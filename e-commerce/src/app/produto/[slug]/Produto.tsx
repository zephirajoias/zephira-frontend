...
const primeiraDisponivel = (p: ProdutoDetalhe | null) =>
  p?.VARIACOES_PRODUTO.find((v) => v.QT_ESTOQUE > 0)?.CD_VARIACAO ?? null;

// `inicial` vem do servidor (page.tsx): a página abre com o produto pronto.
// Sem ele (API falhou no servidor), busca pelo navegador como antes.
export default function ProdutoDetalhePage({
  inicial,
}: {
  inicial: ProdutoDetalhe | null;
}) {
  ...
  const [produto, setProduto] = useState<ProdutoDetalhe | null>(inicial);
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      try {
        ...
        if (!ativo) return;
        setProduto(res);
        setVariacaoSelecionada(primeiraDisponivel(res));
      } catch (err) {
        if (!ativo) return;
